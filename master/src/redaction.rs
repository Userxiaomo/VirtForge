...
mod tests {
    ...
    fn redacts_url_userinfo_credentials() {
        let redacted = redact_text(
            "failed to connect to postgres://vps:db-secret@postgres:5432/vps and https://admin:panel-secret@example.com/api",
        );

        assert!(!redacted.contains("db-secret"));
        assert!(!redacted.contains("panel-secret"));
    ...
    fn redacts_cookie_headers_without_leaking_later_cookie_values() {
        let redacted = redact_text(
            "Cookie: admin_session=session-secret; csrf_token=csrf-secret\nnext header",
        );
    ...
    fn redacts_sensitive_json_fields() {
        ...
        assert_eq!(redacted["bootstrap_token"], REDACTED);
        assert_eq!(redacted["nested"]["message"], "password=[REDACTED]");
    }

    #[test]
    fn redacts_install_command_json_fields() {
        let value = serde_json::json!({
            "node_id": "node-1",
            "install_command": "sudo bash install-agent.sh --bootstrap-token bt_install_secret"
        });

        let redacted = redact_json_value(value);

        assert_eq!(redacted["install_command"], REDACTED);
        assert!(!redacted.to_string().contains("bt_install_secret"));
    }
}
